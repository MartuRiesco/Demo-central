...
function UsersList() {
  ...
  const deleteUser = async (userId)=>{
    try {
      ...
      const response = await axios.delete('https://demo-central-production.up.railway.app/api/admin/delete-user', 
      { data: {
        userId: userId
      }},{
        headers: {
  ...
  const getUsersData = async () => {
    try {
        dispatch(showLoading());
        const response = await axios.get('https://demo-central-production.up.railway.app/api/admin/get-all-users', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
